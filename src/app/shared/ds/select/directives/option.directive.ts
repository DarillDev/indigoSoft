...
export class OptionDirective<T = unknown> implements ISelectOption<T> {
  ...
  public readonly valueInput = input.required<T>({ alias: 'value' });
  public readonly disabledInput = input(false, { alias: 'disabled', transform: booleanAttribute });

  public readonly optionId = `ds-option-${nextOptionId++}`;
  ...
  public readonly selected = computed(() => this.select.isSelected(this.valueInput()));
  ...
  protected handleClick(): void {
    ...
    this.select.selectOption(this.valueInput());
  }
}
